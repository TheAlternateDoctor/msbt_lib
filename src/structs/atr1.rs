...
#[derive(Debug, Clone)]
pub struct ATR1 {
    // Not enough data, since only Rhythm Heaven Megamix has been used for research.
    _magic: Vec<u8>,
...
impl ATR1 {
    pub fn read_from<R: Read + Seek>(buffer: &mut R, order: bytestream::ByteOrder) -> Result<ATR1> {
        ...
        buffer.seek(SeekFrom::Start(
            block_start + 0x10 + section_size as u64 + (0x10 - (section_size % 0x10)) as u64,
        ))?;
