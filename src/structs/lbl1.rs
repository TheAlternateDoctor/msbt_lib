...
impl LBL1 {
    pub fn read_from<R: Read + Seek>(buffer: &mut R, order: bytestream::ByteOrder) -> Result<LBL1> {
        ...
        let leftover_padding = if section_size % 0x10 != 0 {
            0x10 - (section_size % 0x10)
        } else {
            0
        };
        buffer.seek(SeekFrom::Start(
            block_start + 0x10 + section_size as u64 + (leftover_padding) as u64,
        ))?;
    ...
    pub fn write_binary(
        msbt_strings: Vec<MSBTString>,
        order: bytestream::ByteOrder,
    ) -> Result<Vec<u8>> {
        println!("Formatting labels...");
        let mut result = Vec::<u8>::new();
        let mut labels = Vec::<Label>::new();
        let mut label_defs = vec![
            LabelDef {
                amount: 0,
                offset: 0
            };
        ...
        for label in labels {
            if label.hash != current_hash {
                ...
                label_def = LabelDef {
                    amount: 0,
                    offset: base_offset + labels_raw.len() as u32,
                };
            ...
            match order {
                ByteOrder::BigEndian => {
                    labels_raw.append(&mut label.string_index.to_be_bytes().to_vec());
                }
                ByteOrder::LittleEndian => {
                    labels_raw.append(&mut label.string_index.to_le_bytes().to_vec());
                }
            }
        }
        label_defs.insert(current_hash as usize, label_def);
        ...
        for _i in 0..empties {
            match order {
                ByteOrder::BigEndian => {
                    label_defs_raw.append(&mut 0u32.to_be_bytes().to_vec());
                    label_defs_raw
                        .append(&mut (labels_raw.len() as u32 + 0x32Cu32).to_be_bytes().to_vec());
                }
                ByteOrder::LittleEndian => {
                    label_defs_raw.append(&mut 0u32.to_le_bytes().to_vec());
                    label_defs_raw
                        .append(&mut (labels_raw.len() as u32 + 0x32Cu32).to_le_bytes().to_vec());
                }
            }
        }
